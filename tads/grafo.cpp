...
public:
    ...
    void dfsTopo(int nodo, bool visitado[], Cola<int> &cola, Lista<Lista<int> *> &adyacencias)
    {
        visitado[nodo] = true;

        // Obtener la lista de adyacencia del nodo actual
        IteradorLista<Lista<int> *> itAdyacencia = adyacencias.obtenerIterador();
        for (int i = 0; i < nodo; i++)
        {
            itAdyacencia.Avanzar(); // Avanzar hasta la lista de adyacencia correspondiente al nodo
        }

        Lista<int> *vecinos = itAdyacencia.Elemento();
        IteradorLista<int> itVecinos = vecinos->obtenerIterador();
        while (itVecinos.Elemento() != NULL)
        { // Avanzamos mientras haya elementos
            int vecino = itVecinos.Elemento();
            if (!visitado[vecino])
            {
                dfsTopo(vecino, visitado, cola, adyacencias);
            }
            itVecinos.Avanzar();
        }

        cola.Encolar(nodo); // Encolar el nodo después de procesar sus vecinos
    }

    Lista<int> *ordenTopologico(int tope, Lista<Lista<int> *> &adyacencias)
    {
        bool *visitado = new bool[tope]{false};
        Cola<int> cola; // Usamos tu clase Cola para almacenar nodos temporalmente
        Lista<int> *orden = new Lista<int>();
        ...
        for (int i = 0; i < tope; i++)
        {
            if (!visitado[i])
            {
                dfsTopo(i, visitado, cola, adyacencias);
            }
        }

        // Construir el orden topológico en una lista
        while (!cola.EsVacia())
        {
            orden->InsertarFin(cola.Frente());
            cola.Desencolar();
        }

        delete[] visitado; // Liberar la memoria dinámica de visitado
        return orden;
    }
